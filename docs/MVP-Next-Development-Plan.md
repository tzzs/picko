...
2. 使用非生产 Mac Photos 测试图库采集 host Photos-backed metadata indexing 基线；`scripts/capture-metadata-baseline.sh --photos --confirm-non-production-photos --photos-library-label "Non-production Mac Photos test library" --validate-only 1000 10000 50000` 已在 2026-06-01 本地通过且不会读取图库。正式采集前优先运行 `scripts/prepare-phase-5-host-baseline-capture.sh`，它只验证 evidence 中已有 Passed preflight 并输出正式采集、写回和确定日期/timestamp 的 status 复查命令；显式复现时可带 `--evidence`、`--timestamp` 和 `--date`。
