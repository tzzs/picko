...
1. Picko 使用 PhotoKit 公开的 `PHAsset.location` 读取照片 GPS 坐标，不依赖系统 Photos app 内部的“地点”“回忆”或私有聚类结果。
2. 地点名称使用系统 `CLGeocoder` 在设备侧按需反查，优先展示城市、区县、地标、街区或自然地貌。
3. 反查结果仅做内存缓存，不新增持久化地点名称 schema，不上传照片、坐标或反查结果。
4. 当系统反查失败时，用户界面显示中文兜底名称，例如“附近地点”，不直接展示经纬度。
5. 地图聚合可以继续使用代表坐标定位，但地点命名应优先尝试组内真实照片坐标，避免只用平均坐标导致落点偏移后反查失败。
