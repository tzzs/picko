...
4. Core Data 或 SwiftData 存储本地整理状态。
...
1. MVP 是否必须首发同时支持 iOS 和 macOS，还是先 iOS 后 macOS。
2. 相似度算法第一版使用时间 + 地点 + 缩略图 hash，还是直接引入 Vision 特征。
3. 预删除状态是否需要通过 CloudKit 跨设备同步。
4. 付费墙应该放在相似组数量限制、处理照片数量限制，还是高级 AI 推荐上。
5. 中文品牌名是否确定为“拾影”，英文名是否确定为 “Picko”。
